...
class instance_custom_training:
    ...
    def modelConfig(self,network_backbone = "resnet101",  num_classes =  1,  class_names = ["BG"], batch_size = 1, detection_threshold = 0.7, image_max_dim = 512, image_min_dim = 512, image_resize_mode ="square", gpu_count = 1, learning_rate=0.001, learning_momentum=0.9):
       ...
       IMAGE_RESIZE_MODE = image_resize_mode,GPU_COUNT = gpu_count)
        ...
        self.config.LEARNING_MOMENTUM = learning_momentum
    ...
    def load_pretrained_model(self, model_path):
        ...
        self.model = modellib.MaskRCNN(mode="training", model_dir = self.model_dir, config = self.config,learning_rate=self.config.LEARNING_RATE, learning_momentum=self.config.LEARNING_MOMENTUM)
    ...
    augmentation = False,csv_logger_filename=None,flipH=0.5,flipV=0.5,blur=5.0,
    reduce_lr_factor=0.2,reduce_lr_patience=10,reduce_lr_min_lr=0.0,
    earlystop_patience=0):
        if augmentation == False:
            ...
            augmentation = imgaug.augmenters.Sometimes(0.5, [
                    imgaug.augmenters.Fliplr(flipH),
                    iaa.Flipud(flipV),
                    imgaug.augmenters.GaussianBlur(sigma=(0.0, blur))
                ])
            ...
            augmentation = augmentation, epochs=num_epochs,layers=layers,
            csv_logger_filename=csv_logger_filename, reduce_lr_factor=reduce_lr_factor,
            reduce_lr_patience=reduce_lr_patience,reduce_lr_min_lr=reduce_lr_min_lr,
            earlystop_patience=earlystop_patience
            )


    def evaluate_model(self, model_path, iou_threshold = 0.5, eval_metrics_filename=None):
        self.model = MaskRCNN(mode = "inference", model_dir = os.getcwd(), config = self.config,learning_rate=self.config.LEARNING_RATE, learning_momentum=self.config.LEARNING_MOMENTUM)  
        ...
        #create csv log header
        log_head = ["Model","mAP(IoU)"]
        log_data = []
        ...
        for modelfile in model_files:
            ...
            #append to csv_log
            log_line = [modelfile,f"{mAP:01f}"]
            log_data.append(log_line)
          
        #save csv log to file
        with open(eval_metrics_filename, 'w') as log_file:
...
def display_instances_2(image, boxes, masks, class_ids,  class_name):
    ...
    for i, color in enumerate(colors):
        ...
        if label == 'ripe':
          ...
          image = apply_mask(image, mask, colors[1])
          print(colors[1])
...
def display_box_instances(image, boxes, masks, class_ids, class_name, scores = None):
    ...
    for i, color in enumerate(colors):
        ...
        if label == 'ripe':
          image = apply_mask(image, mask, RED_COLOR)
        elif label == 'unripe':
          image = apply_mask(image, mask, YELLOW_COLOR)


        color_rec = [int(c) for c in np.array(colors[i]) * 255]
        image = cv2.rectangle(image, (x1, y1), (x2, y2), color_rec, 2)
        font_scale = round((image.shape[0]/2000)*3,2)
        thickness = round((font_scale/3)*10)
        # print("font_scale ",font_scale, "thickness ",thickness)

        image = cv2.putText(
            image, caption, (x1, y1), cv2.FONT_HERSHEY_SIMPLEX,font_scale, color = (255, 255, 255),thickness=thickness)
    return image
